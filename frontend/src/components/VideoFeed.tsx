...
export function VideoFeed({ onCameraStatusChange }: VideoFeedProps) {
  ...
  useEffect(() => {
    if (!useLiveCamera) {
      ...
      // Live camera mode - no need to fetch images
      setLoading(false);
      setError(false);
    }
  }, [useLiveCamera]);
  ...
  const handleCameraStatusChange = (status: 'active' | 'inactive' | 'error') => {
    ...
    // Propagate status change to parent component
    onCameraStatusChange?.(status);
    console.log('[VideoFeed] Camera status changed:', status);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className={`w-3 h-3 rounded-full mr-3 ${
              useLiveCamera ? (cameraStatus === 'active' ? 'bg-green-400' : cameraStatus === 'error' ? 'bg-red-400' : 'bg-yellow-400') : 
              (!error ? 'bg-green-400' : 'bg-red-400')
            } animate-pulse`}></div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center px-3 py-1 bg-white bg-opacity-20 rounded-full">
              <div className={`w-2 h-2 rounded-full mr-2 ${
                useLiveCamera 
                  ? (cameraStatus === 'active' ? 'bg-green-400' : cameraStatus === 'error' ? 'bg-red-400' : 'bg-yellow-400')
                  : (!error ? 'bg-green-400' : 'bg-red-400')
              } animate-pulse`}></div>
              <span className="text-sm font-medium text-white">
                {useLiveCamera 
                  ? (cameraStatus === 'active' ? 'Live Stream' : cameraStatus === 'error' ? 'Error' : 'Offline')
                  : (imageInfo?.source === 'mock_camera' ? 'Mock Camera' : 'API Stream')
                }
              </span>
            </div>
          </div>
      ...
      <div className="p-6">
        {useLiveCamera ? (
          // Live Camera Mode
          <LiveCamera onCameraStatusChange={handleCameraStatusChange} />
        ) : (
          // Backend API Mode
          <>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              <span className="font-medium">Current Mode:</span> {useLiveCamera ? 'Live Camera (Direct Webcam Access)' : 'Backend API (Server-Side Processing)'}
            </div>
            <div className="text-xs text-gray-500">
              {useLiveCamera 
                ? 'Uses browser camera API for real-time feed'
                : 'Uses backend server for image processing'
              }
            </div>
          </div>
        </div>
