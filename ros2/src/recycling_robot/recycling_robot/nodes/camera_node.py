...
class CameraNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_rate', 0.1)  # 10 FPS = 0.1s interval
        ...
        self.snapshot_enabled = self.get_parameter('snapshot_enabled').value
        ...
        if self.camera_connected:
            self.timer = self.create_timer(self.publish_rate, self.capture_and_publish_frame)
            self.get_logger().info('[Camera] Camera node started successfully')
            self.get_logger().info(f'[Camera] Publishing at {1.0/self.publish_rate:.1f} FPS')
        else:
        ...
        if self.snapshot_enabled:
            self.get_logger().info(f'[Camera] Snapshot path: {self.snapshot_path}')
        ...
        try:
            ...
            for attempt in range(self.retry_attempts):
                try:
                    ...
                    self.get_logger().info(f'[Camera] ✅ Logitech C270 initialized successfully')
                    return True
    ...
    def _configure_camera(self):
        """Configure camera properties for Logitech C270"""
        try:
            # Set resolution
            ...
            # Try to set MJPEG format (Logitech C270 preferred)
            self.camera.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('M','J','P','G'))

            # Enable auto-exposure (value 0.75 typically enables auto-exposure on most cameras)
            self.camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.75)

            # Set buffer size to 1 to get latest frames
            self.camera.set(cv2.CAP_PROP_BUFFERSIZE, 1)

            # Additional Logitech C270 optimizations
            self.camera.set(cv2.CAP_PROP_AUTOFOCUS, 1)  # Enable auto-focus if supported
    ...
    def _log_camera_info(self):
        ...
        try:
            ...
            self.get_logger().info(f'[Camera] Resolution: {actual_width}x{actual_height} (requested: {self.width}x{self.height})')
            self.get_logger().info(f'[Camera] FPS: {actual_fps} (requested: {self.fps})')
    ...
    def _write_snapshot_atomic(self, image_bytes: bytes, format_type: str):
        """Write snapshot with atomic operation and fallback support"""
        try:
            ...
            self.get_logger().debug(f'[Camera] Snapshot saved: {actual_path} ({len(image_bytes)} bytes, {format_type})')
            
        except Exception as e:
            self.get_logger().error(f'[Camera] Failed to write snapshot: {e}')

    def capture_and_publish_frame(self):
        """Main method to capture and publish frames"""
        try:
            # Skip publishing if pipeline is busy
            if self.pipeline_state == "processing":
                self.get_logger().debug('⏸️ [Camera] Pipeline busy, skipping frame capture (waiting for sorting to complete)')
                return
            ...
            if not self.camera_connected:
                ...
                if current_time - self.last_reconnect_attempt > self.reconnect_cooldown:
                    self.get_logger().info('[Camera] Attempting to reconnect camera...')
                    self._initialize_camera()
            ...
            if frame is None:
                self.get_logger().warn('[Camera] No frame captured, camera may be disconnected')
                self.camera_connected = False
                return

            # Increment frame counter
            self.frame_counter += 1
            frame_name = f'frame_{self.frame_counter:06d}.jpg'
            ...
            try:
                ...
                format_type = 'JPEG'
                self.get_logger().debug(f'[Camera] JPEG encoding successful ({len(image_bytes)} bytes)')
                ...
                try:
                    ...
                    image_bytes = buf.getvalue()
                    format_type = 'PNG'
                    self.get_logger().info(f'[Camera] PNG fallback successful ({len(image_bytes)} bytes)')
            ...
            self.get_logger().info(f'[Camera] Published to /pipeline/image_raw: {frame_name} ({len(image_bytes)} bytes, {format_type})')

        except Exception as e:
            self.get_logger().error(f'[Camera] Failed to capture and publish frame: {e}')
            # Mark camera as disconnected on persistent errors
            self.camera_connected = False
    ...
    def _retry_camera_initialization(self):
        ...
        if not self.camera_connected:
            ...
            if self._initialize_camera():
                # Successfully reconnected, switch to normal capture timer
                self.timer.cancel()
                self.timer = self.create_timer(self.publish_rate, self.capture_and_publish_frame)
                self.get_logger().info('[Camera] Camera reconnected, resuming normal operation')
